...
def L_model_forward(X, parameters):
    ...
    caches.append(cache)
    ### END CODE HERE ###
#     print('X.shape[1]:',X.shape[1],'X.shape[0]:',X.shape[0])
...
#     print('AL:', np.log(AL), 'Y', (Y))
    for l in range(1, L):
         w_squared=np.sum(np.square(parameters['W{}'.format(l)]))
         L2_regularization_cost=L2_regularization_cost+w_squared
    L2_regularization_cost=L2_regularization_cost*lambd/(2*m)
    # Compute loss from aL and y.
    cost = (-sum(np.dot(Y,np.log(AL).T)+np.dot((1-Y),np.log(1-AL).T))/m)+L2_regularization_cost
...
def predict(X_test, Y, parameters):
    """
    ...
    ### START CODE HERE ### (≈ 2 lines of code)
    A2, cache = L_model_forward(X_test, parameters)   
